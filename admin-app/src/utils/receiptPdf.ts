...
export function generateReceiptPdf(
  ...
  const rowH = 26;
  function tableHeader(cols: { label: string; align?: 'left' | 'right' }[], widths: number[]) {
    doc.setFillColor(...indigo);
    ...
    let x = margin + 10;
    cols.forEach((col, i) => {
      if (col.align === 'right') doc.text(col.label, margin + widths.slice(0, i + 1).reduce((a, b) => a + b, 0) - 10, y + 15, { align: 'right' });
      else doc.text(col.label, x, y + 15);
      x += widths[i];
    });
    y += 22;
  }
  ...
  tableHeader(
    [{ label: 'Particulars' }, { label: 'Amount (INR)', align: 'right' }],
...
export function downloadReceiptPdf(receipt: CombinedReceipt, school: { name: string; address: string; phone: string }) {
  ...
  doc.save(`${receipt.receiptNo.replace(/\//g, '-')}-${INSTALLMENT_LABEL[receipt.installmentId]?.replace(/\s+/g, '-') ?? receipt.installmentId}.pdf`);
}
...
export function printReceiptPdf(receipt: CombinedReceipt, school: { name: string; address: string; phone: string }) {
  const doc = generateReceiptPdf(receipt, school);
  doc.autoPrint();
  window.open(doc.output('bloburl'), '_blank');
}
