...
// array (grading, which returns one entry per question) - whichever bracket
// type opens FIRST in the response is the real outer shape, so that one is
// used to find the matching close, instead of always assuming an object and
// (for an array response) slicing off its enclosing `[`/`]` and leaving a
// comma-joined list of objects that isn't valid JSON on its own. Mirrors the
// student-app copy of this function (see prompt.ts for why this stays a
// hand-synced client-side copy).
